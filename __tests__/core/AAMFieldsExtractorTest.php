...
final class AAMFieldsExtractorTest extends FacebookWordpressTestBase {
	/**
	 * Test that get_normalized_user_data returns the expected normalized data when AAM is enabled.
	 *
	 * @return void
	 */
	...
	/**
	 * Verifies that get_normalized_user_data() returns an array with only the fields enabled in the AAM settings.
	 *
	 * This test runs 25 times, with each iteration testing a different subset of fields.
	 * The subset of fields is randomly generated and used to set the enabled fields in the AAM settings.
	 * The test then verifies that the returned array only contains the fields that were enabled in the AAM settings.
	 */
	...
	/**
	 * Tests that get_normalized_user_data returns an empty array when AAM is disabled.
	 *
	 * This test verifies that when AAM is disabled, the normalized user data array is empty.
	 *
	 * @return void
	 */
	...
	/**
	 * Test that get_normalized_user_data returns an empty array when AAM is not present.
	 *
	 * This test verifies that when AAM is not present in the AAM settings, the normalized user data array is empty.
	 */
	...
	/**
	 * Asserts that the user data array only contains the fields specified in the fields subset.
	 *
	...
	private function assertOnlyRequestedFieldsPresentInUserDataArray(
		...
		$this->assertEquals( count( $fields_subset ), count( $user_data_array ) );
	 ...
	 *
	 * This is used by the testReturnsArrayWithRequestedUserDataWhenAamEnabled test to
	 ...
	 *
	 * @param string $pixel_id The pixel ID to set in the AAM settings.
	 * @param bool   $enable_aam Whether to enable automatic matching.
	 * @param array  $enable_aam_fields The fields to enable for automatic matching.
	 *
	 ...
	 *
	 * This function returns a sample user data array with all the fields that can be used in automatic matching.
	 *
