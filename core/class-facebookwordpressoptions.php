...
class FacebookWordpressOptions {
	 ...
	 *
	 * It sets the options, version info, AAM settings, user info, CAPI integration status,
	 * CAPI integration events filter, and CAPI PII caching status.
	 */
	 ...
	 *
	 * This function returns the value of the CAPI PII caching status from the WordPress
	 * options. If the value is not set, it returns the default value.
	 *
	 ...
	 *
	 * Retrieves the value of the CAPI integration events filter from the WordPress
	 * options and assigns it to the class property $capi_integration_events_filter.
	 */
	 ...
	 *
	 * This function returns the value of the CAPI integration events filter from the WordPress
	 * options. If the value is not set, it returns the default value.
	 *
	 ...
	 *
	 * This function is a convenience method to check if the PageView event is filtered
	 * in the CAPI integration events filter. It returns true if the PageView event is
	 * filtered, false otherwise.
	 *
	 * @return bool If PageView is filtered in the CAPI integration events filter.
	 */
	 ...
	 *
	 * If the default pixel ID is not set, this function returns an empty string.
	 *
	 ...
	 *
	 * If the default access token is not set, this function returns an empty string.
	 *
	...
	/**
	 * Retrieves the default external business ID, which is a unique ID generated by
	 * prepending the current time to the default external business ID prefix.
	 *
	 ...
	 * This method is called by the Plugin class in its constructor.
	 *
	 * It sets the options by first checking if the new options are saved in WP database,
	 * if so, they are used. If not, the old options are used. If the old options are not
	 * present, the default values are used.
	 */
	 ...
	 *
	 * If the pixel ID is not set in the options, the default pixel ID is returned.
	 *
	 ...
	 *
	 * If the external business ID is not set in the options, the default external
	...
	/**
	 * Retrieves the status indicating whether the Facebook Business Extension is installed.
	 *
	 * If the FBE installed status is not set in the options, the default FBE installed
	 * status is returned.
	 *
	 ...
	 *
	 * If the access token is not set in the options, the default access token is returned.
	 *
	 ...
	 *
	 * This function is called when the 'init' action is triggered, and it sets the user
	 * information stored in the class. The user information is an array that may contain
	 * keys like 'email', 'first_name', and 'last_name', depending on the data available
	...
	public static function register_user_info() {
		...
		if ( 0 === $current_user->ID ) {
			...
			self::$user_info = AAMFieldsExtractor::get_normalized_user_data( $user_info );
		}
	}
	 ...
	 *
	 * The version information is an array that contains the keys 'pluginVersion'
	 * and 'source'. The 'pluginVersion' key contains the current version of the
	 * plugin, and the 'source' key contains the source of the plugin.
	 *
	 ...
	 *
	 * The version information is an array that contains the keys 'pluginVersion', 'source',
	 * and 'version'. The 'pluginVersion' key contains the current version of the plugin,
	 * the 'source' key contains the source of the plugin, and the 'version' key contains
	 ...
	 *
	 * @return AdsPixelSettings The AdsPixelSettings object containing the AAM settings.
	 */
	 ...
	 * WordPress database. If there are, they are converted into an AdsPixelSettings
	 * object and returned. If there are no cached settings, the method fetches
	 * the settings from the Facebook domain and caches them in the WordPress
	 ...
	 * Otherwise, the AAM settings are set to disable automatic matching and all its fields.
	 */
	 ...
	 *
	 * This method initializes the AAM settings to null and checks if a pixel ID is set.
	 * If no pixel ID is found, the method returns early. If the Facebook Business Extension
	 * (FBE) is installed, the AAM settings are set using the FBE-based settings.
