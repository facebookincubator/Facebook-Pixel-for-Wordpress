...
class FacebookWordpressPixelInjection {
	 ...
	 * using the `wp_head` and `wp_footer` actions. It also injects the necessary
	 * code for the no-JavaScript version of the Facebook Pixel.
	 *
	 * @return void
	 */
	...
	/**
	 * Injects the Facebook pixel base code, Open Bridge configuration code if CAI is enabled,
	 * Facebook pixel initialization code and Facebook pixel page view code.
	 *
	 * This method is hooked into the `wp_head` action and is responsible for injecting the
	 * necessary code to enable the Facebook pixel for the current page. It uses the
	 * `FacebookPixel` class to generate the necessary code and injects it into the page.
	 *
	 * @return void
	 */
	public function inject_pixel_code() {
		...
		if (
		( isset( self::$render_cache[ FacebookPluginConfig::IS_PIXEL_RENDERED ] ) &&
