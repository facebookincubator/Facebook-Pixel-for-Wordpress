...
class FacebookWordpressOpenBridge {
	 ...
	 *
	 * @return FacebookWordpressOpenBridge The instance of FacebookWordpressOpenBridge class.
	 */
	...
	public function extract_from_databag( $databag ) {
		...
		$event_data = array(
			...
			'external_id'      => self::get_external_id( $current_user, $databag ),
			...
			'content_name'     => self::get_custom_data( 'content_name', $databag ),
			'content_ids'      => self::get_custom_data_array( 'content_ids', $databag ),
			'content_category' =>
	 ...
	 *
	 * If the user data contains an email, use that. Otherwise use the email from the AAM settings.
	 *
	 ...
	 *
	 * If the user data contains a first name, use that. Otherwise use the first name from the AAM settings.
	 *
	 * @param array $current_user_data The user data.
	 * @param array $pixel_data The AAM settings.
	 *
	...
	private static function get_first_name( $current_user_data, $pixel_data ) {
		...
		return self::get_aam_field( AAMSettingsFields::FIRST_NAME, $pixel_data );
	}

	/**
	 * Get the user's last name.
	 *
	 * If the user data contains a last name, use that. Otherwise use the last name from the AAM settings.
	 *
	 * @param array $current_user_data The user data.
	 * @param array $pixel_data The AAM settings.
	 *
	 ...
	 *
	 * If the user data contains an ID, use that. Otherwise use the external ID from the AAM settings.
	 ...
	 *
	 * If the user data contains a zip code, use that. Otherwise use the zip code from the AAM settings.
	 *
	 * @param array $current_user_data The user data.
	 * @param array $pixel_data The AAM settings.
	 *
	 ...
	 *
	 * If the country is set in the current user data, use that. Otherwise use the
	 * value from the AAM settings.
	 *
	 * @param array $current_user_data The user data.
	 * @param array $pixel_data The AAM settings.
	 ...
	 *
	 * If the user data contains a state, use that. Otherwise use the state from the AAM settings.
	 *
	 * @param array $current_user_data The user data.
	 * @param array $pixel_data The AAM settings.
	 *
	 ...
	 * @param array  $pixel_data The array containing the advanced matching settings.
	 *
	 * @return string The value associated with the given key if found, otherwise an empty string.
	 */
	 ...
	 *
	 * This function checks if the custom data label exists within the pixel data.
	 * If the specified key is found, it returns the corresponding custom data array.
	 * If the key is not found, it returns an empty array.
	 *
	 * @param string $key The key to retrieve the custom data array for.
	 * @param array  $pixel_data The array containing the custom data.
	 *
	 * @return array|string The custom data array if the key is found, otherwise an empty array.
	 */
